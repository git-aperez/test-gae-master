...
public class TestBeanDAO {
    ...
    public List<TestBean> list(String filtro, String campo) {
        LOGGER.info("Retrieving list of beans by " + filtro + ": " + campo);
        
        if ("autor".equals(filtro)) {
        	return ObjectifyService.ofy().load().type(TestBean.class).filter("autor >=", campo).filter("autor <", campo + "\uFFFD").list();	
        }
        else {
        	return ObjectifyService.ofy().load().type(TestBean.class).filter("nombre >=", campo).filter("nombre <", campo + "\uFFFD").list();
        }
